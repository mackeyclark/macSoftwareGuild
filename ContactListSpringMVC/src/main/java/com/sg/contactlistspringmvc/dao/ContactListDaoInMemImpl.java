...
public class ContactListDaoInMemImpl implements ContactListDao {
    ...
    //holds all our contact objects - simulates the database
    private Map<Long, Contact> contactMap = new HashMap<>();
    //used to assaign ids to contacts - simulates the auto increment primary key
    //for contacts in a database
    public static long contactIdCounter = 0;
    ...
    public List<Contact> searchContacts(Map<SearchTerm, String> criteria) {
        ...
        //declare all the predicate conditions - remember that predicate is a
        //functional interface with one method called test(T t) that returns a
        //boolean. Since predicate is generic we get to specify the type of
        //object we want T to be - in our case, we want T to be of type Contact.
        //That means the predicates declared here will have one method:
        ...
        //placeholder predicate - always return true. Used for search terms that
        //are empty - if the user didn't specify a value for one of the search
        //terms, we must return true because we are ANDing all of the search 
        //terms together and our spec says that we return everything in the DAO
        //when the user leaves all the search terms blank.
        ...
        //only returns true when it finds a match for the given term.
        ...
        //return the list of contacts that match the given criteria. To do this
        //we just AND all the predicates together in a filter operation
